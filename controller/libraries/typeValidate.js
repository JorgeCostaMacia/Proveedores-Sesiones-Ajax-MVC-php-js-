...
function esTextoEspacioPuntoComaBarraGuionNumero(dato){
    let controlNumero = true;
    if(dato != "" && dato != null){
        for(let i = 0; i < dato.length; i++){
            if(!esTexto(dato[i]) && !esEspacio(dato[i]) && !esPunto(dato[i]) && !esComa(dato[i]) && !esNumero(dato[i]) && !esGuion(dato[i]) && !esBarra(dato[i])){ controlNumero = false; i = dato.length; }
        }
    } else { controlNumero = false; }
    return controlNumero;
}
